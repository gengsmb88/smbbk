...
class Dana {
    ...
    const user_agent 				= 'DANA/16820 Skywalker/1.0 with CFNetwork/1220.1 Darwin/20.3.0';
    const action_mark 				= 'DANA Wallet';
	const client_id					= "dana_ios";
    ...
    */
	...
	//-----------------------------------------------------------------------------------------------------------------------
	// Informasi
	public function login2FA(String $pin = '') {
		$post_params = array(
            'nohp'				=> $this->acc_num,
		...
		$url_api = sprintf("%s/%s", self::api_url, 'api/account/login');
	...
	public function loginVerify(String $reference_id, String $otp_code) {
		...
		$url_api = sprintf("%s/%s", self::api_url, 'api/account/otp');
		$this->set_curl_init($url_api, $this->create_curl_headers($this->headers));
		$this->set_curl_body($post_params);
		try {
			$curl_collect = $this->curlexec();
	...
	public function get_account_data(String $token, Array $session_params = array()) {
		...
		$url_api = sprintf("%s/%s/%s", self::api_url, 'api/account/balance', $this->acc_num);
		$this->set_curl_init($url_api, $this->create_curl_headers($this->headers));
	...
	public function get_dana_transaction_history(String $token, Array $session_params, Array $daterange_params) {
		$this->set_authorization($token);
		if (!isset($session_params['device_id']) || !isset($session_params['push_notif_id'])) {
			return false;
		}
		if (!isset($daterange_params['starting']) || !isset($daterange_params['stopping'])) {
			return false;
		}
		
		$post_params = array(
            'nohp'				=> $this->acc_num,
			'start_date'		=> $daterange_params['starting'],
            'end_date' 			=> $daterange_params['stopping'],
        );
		
		$url_api = sprintf("%s/%s", self::api_url, 'api/account/history/date');
		$this->set_curl_init($url_api, $this->create_curl_headers($this->headers));
		$this->set_curl_body($post_params);
		try {
			$curl_collect = $this->curlexec();
	...
	public function transfer_set_wallet(Array $input_params, String $transfer_step = 'validate') {
		...
		if (!isset($input_params['transfer_instance'])) {
			...
			$transfer_instance = (is_string($input_params['transfer_instance']) ? strtolower(trim($input_params['transfer_instance'])) : '');
			if (!in_array($transfer_instance, ['dana', 'wallet', 'bank'])) {
				return false;
			}
		}
		if (!isset($input_params['transfer_amount']) || !isset($input_params['transfer_number'])) {
	...
	private function set_curl_body_with_integer_check($post_params = FALSE) {
		if ($post_params === FALSE) {
			...
			$post_data = '';
			if (is_array($post_params)) {
				$post_data = http_build_query($post_params, JSON_NUMERIC_CHECK);
			} else if (is_object($post_params)) {
				$post_data = http_build_query($post_params, JSON_NUMERIC_CHECK);
			} else if (is_string($post_params) || is_numeric($post_params)) {
	...
	private function set_curl_body_get($post_params = FALSE) {
		if ($post_params === FALSE) {
			curl_setopt($this->ch, CURLOPT_CUSTOMREQUEST, 'GET');
			curl_setopt($this->ch, CURLOPT_POST, FALSE);
			curl_setopt($this->ch, CURLOPT_HTTPGET, TRUE);
